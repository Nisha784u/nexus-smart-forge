...
/**
 * Registry of the projects loaded for the signed-in user's workspace.
 */
const projectRegistry = new Map<string, Project>();

export function setProjectRegistry(list: Project[]) {
  projectRegistry.clear();
  for (const p of list) projectRegistry.set(p.id, p);
}
